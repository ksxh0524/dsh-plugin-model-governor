...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			// footer 全局区：list 槽无 key，注册即按序渲染。
			ctx.effect(function () {
				return ctx.slots.inject("settings.models.footer", function () {
					return ctx.slots.register({ name: "settings.models.footer" }, function FooterSlot() {
