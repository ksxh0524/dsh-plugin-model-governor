 ...
 *  - **语义诚实**：本卡写的是运行时 live 配置（bundle 行 config 是静态起点，重启回落，见 src/cordis.ts:43-44），
 *    不落设置文档——行下常显一句「仅本次运行」，别让它长得像会持久化的设置项。
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CONTRIBUTION = {
			...
			descriptors: [
				descriptor("describe", "filter", { file: "src/cordis.ts", line: 271, column: 9 }),
				descriptor("configure", "patch", { file: "src/cordis.ts", line: 305, column: 9 }),
				descriptor("probe", "spec", { file: "src/cordis.ts", line: 335, column: 9 }),
				descriptor("probeStatus", "filter", { file: "src/cordis.ts", line: 404, column: 9 }),
				descriptor("cancelProbe", "target", { file: "src/cordis.ts", line: 437, column: 9 }),
			],
		};
		...
		var CSS = [
			...
			".gvr-row{display:flex;align-items:center;gap:8px;flex-wrap:wrap}",
		...
		function Seat(props) {
			return h("div", { className: "gvr-seat" }, props.children);
		}
		...
		function ActionButton(props) {
			...
			return h(
				"button",
				{ type: "button", className: "gvr-text", disabled: props.disabled, onClick: props.onClick, title: props.title, "aria-label": props.ariaLabel },
				props.children,
			);
		}
		...
		function GovRow(props) {
			...
			return h(
				...
				// 语义诚实：写的是运行时 live 配置，不落设置文档——不写清就长得像会持久化的设置项。
				h("p", { className: "gvr-note" }, "仅本次运行生效，host 重启回落 bundle 配置。"),
			);
		}
